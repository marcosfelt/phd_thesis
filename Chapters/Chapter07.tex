...
\begin{table}
    ...
    Test MIX & 5 985 078& DG & 0.11 & 0.10 \\
              && DG-TLCB & 0.10 & 0.10 \\
    Valid CONT & 111 426 & DG & 0.027 & 0.026 \\
              && DG-TLCB & 0.033 & 0.032 \\
...
Results of training on the COSMO-RS activity coefficient dataset are shown in Table \ref{tab:cosmo_rs_results}. The base \textit{DeepGamma} model  and model pretrained on COSMO-RS data 
 achieve similar mean absolute errors. Similar mean absolute errors are achieved on the validation and test datasets for all models. As expected, the Valid CONT holdout set has the lowest MAE due to the same molecules being in the training set and holdout set.  For the best performing base model, the INDP and MIX datasets have similar error profiles, indication that model has learned a good encoder representation.  Note that even though the transfer learning model has approximately the performance as the base models, it takes ~50 \% less time to train. 
