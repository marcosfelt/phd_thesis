...
In this work, we introduce use multifidelity Bayesian optimization to accelerate controller tuning. Specifically, we develop a dynamic simulation of a distillation column that is only a low fidelity approximation of the original distillation experiment. We then use multifidelity Bayesian optimizaiton algorithm can use this simulation to reduce the number of iterations required to find optimal controller parameters. 
...
Controller tuning involves identifying the three parameters of the PID equation, $K^P$, $\tau_I$ and $\tau_D$:
...
We formulate the problem of controller design and tuning as an optimization problem. The aim is to find a set of controller parameters that will achieve desired behavior. We define good controller behavior as minimizing the integral absolute error (IAE) for each controlled variable and minimizing total controller movement (CM):
\begin{equation}
