...
\begin{table}
    ...
    \begin{tabular}{cc}
        ...
        \hline
         ...
         $L_i$ & Liquid flow leaving stage $i$  \\
         $V_i$  & Vapor flow leaving stage $i$ \\
...
The reboiler and sump are lumped into into one stage. 
...
For liquid enthalpy $h^{L,c}$ of each component, take a mole fraction weighted average of the specific enthalpies calculated by integrating a constant heat capacity from a reference temperature $T_{ref}$ of normal boiling point of each component $c$. Data is taken from Table 2-72 of Perry’s Chemical Engineer's Handbook \cite{Perrys2018}.
...
Based on measurements from BASF, I assume the diameter of the weir $w$ is 8 mm and the height of the weir $h$ is 10 mm.  I then take $h = 1\cdot10^{-2} m$ and $A=\pi w^2/4 = \pi(8 \cdot 10^{-3})^2/4 = 5.024 \cdot 10^{-5} m^2$, and g is the gravity of Earth. Therefore $W_{tray}^{init}=120149 \; m^{-3/2}s^{-1}$. $n^v_{ref}=Ah=5.024 \cdot 10^{-7} m^3$. 
...
In Figure \ref{fig:comparison_controlbo}, I create a performance metric to compare the final tuning of each optimisation strategy. The metric is as follows:  
