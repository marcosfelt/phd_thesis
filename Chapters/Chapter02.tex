...
\begin{enumerate}
\item \textbf{Felton K.}*, Rittig J.* and Lapkin A. \textif{Chemistry Methods}. "Summit: Benchmarking Machine Learning Methods for Reaction optimization." 2021.
...
Summit has several key utilities:
...
% As shown in \ref{fig:benchmarks}B, our second benchmark is the optimization of a Pd-catalysed C-N cross coupling reaction. The aim is to select the best of three catalysts (t-BuXPhos, t-BuBrettPhos, AlPhos) and four organic bases (TEA, TMG, BTMG, and DBU) for the reaction as Ill as the residence time (1-30 minutes), temperature (30-100 $^{\circ}$C) and base equivalents (1.0-2.5). Since it is difficult to find fundamental kinetic models that span across multiple catalysts and reagents, I build a predictive model on data from 96 reactions published by Baumgartner et al.\cite{Baumgartner2019} As shown in Figure S7, the trend of the data is captured by the model, and the cross-validation results show that the model predicts yield on unseen reaction conditions with approximately 8\% mean absolute error.  I use the trained model as a virtual experiment that the optimization strategies consult for yield at given conditions. In addition to optimising yield, I also minimise cost of the reagents, so this becomes a multi-objective optimization.
 
I note that I set the bounds on the decision variables by observing the limits of the experimental data from the original sources for both benchmarks. When optimising a new reaction the lab, a chemist's prior knowledge and a small number of scoping experiments could be used to set these bounds judiciously. For example, to prevent precipitation in a flow reactor, a researcher could chose bounds based on a small number of solubility experiments.

\subsubsection{Mechanistic Benchmarks}

% As shown in \ref{fig:benchmarks}A, the first benchmark is the optimization of a nucleophilic aromatic substitution (S\textsubscript{N}Ar) reaction in a flow reactor. The benchmark is based on a kinetic model developed by Hone et al.,\cite{Hone2017} for the reaction of difluoronitrobenzene \textbf{1} and pyrrolidine \textbf{2}, which produces one desired product and two side products. The reaction is carried out in a virtual flow reactor, so the natural objective is to maximise space time yield (STY), the mass of product formed per unit residence time. Additionally, I minimise the E-factor, which is defined as the ratio of product formed to waste produced.\cite{Sheldon2018} 
...
\begin{figure}
    ...
    \subfigure[]{
        \centering
        \includegraphics[width=0.5\textwidth]{gfx/Chapter02/flow_snar}
        \subcaption{}
    }
    \subfigure[]{
        \centering
        \includegraphics[width=0.5\textwidth]{gfx/Chapter02/pareto_front_snar.png}
        \subcaption{}
    }
...
\begin{figure}[p]
    \centering
    \includegraphics[width=0.9\textwidth]{gfx/Chapter02/c_n_benchmarks_thesis_2.png}
...
% \begin{figure}[p]
%     \centering
%     \includegraphics[width=0.9\textwidth]{gfx/Chapter02/suzuki_benchmarks_thesis.png}
...
\begin{figure}
    \subfigure[]{
        \centering
        \includegraphics[width=0.5\linewidth]{gfx/Chapter02/custom_asf.png}
        
    }
    \subfigure[]{
        \centering
        \includegraphics[width=0.5\linewidth]{gfx/Chapter02/categorical_transform.png}
...
\section{Summit: Benchmarking Machine Learning for Reaction Optimization}
